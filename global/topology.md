...
| `src/state/` | `_sta` | All mutables, literals, constants, config values — single source of truth (except Adapter) |
...
RULE: All mutables, literals, and constants live in `src/state/` — no magic numbers or hardcoded values in other layers
RULE: Adapter is the sole exception — it owns its own state as the data exchange hub
RULE: Config structs (`_cfg`) and state structs (`_sta`) both live in `src/state/`
BANNED: Magic numbers, hardcoded strings, or mutable state in `_ui`, `_core`, `_pal`, `_gtw`
BANNED: Literals scattered across layers — centralize in `src/state/`
...
GUI └──events──────────────►│              PAL  ──abstracts──►  Platform
                             ...
                             ▼
                          State (_sta)  ◄──read/write──  Adapter
                             │
                          Gateway ──IO──►  (loads/saves _sta to disk)
                             │              ▲
                             └──────────────┘
```
...
|-------------|----------------------|
| `_ui` | `_core`, `_pal`, `_gtw`, `_sta` — must route through `_adp` |
| `_core` | `_adp`, `_ui`, `_gtw`, `_sta` — Core is pure; calls only `_pal` |
| `_sta` | `_adp`, `_ui`, `_core`, `_gtw` — State structs are data-only, no logic imports |
| `_pal` | `_core`, `_adp`, `_ui`, `_gtw`, `_sta` — PAL is the bottom layer |
| `_gtw` | `_adp`, `_ui` — Gateway reads `_sta`; calls `_pal` for IO |
| `_adp` | *(hub — may reference all layers including `_sta`)* |
