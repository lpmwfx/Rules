...
related: [slint/rust-bridge.md, global/config-driven.md]
keywords: [global, singleton, Colors, Spacing, Type, Strings, AppBridge, in-property, ui-global, set-dark-mode, localization, per-instance]
layer: 3
---
# Slint Globals
...
RULE: Use globals for design tokens (`Colors`, `Spacing`, `Type`), localization strings (`Strings`), and shared event routing (`AppBridge`)
...
RULE: Token globals (`Colors`, `Spacing`) use `out property` for computed values that read a single `in property` (e.g. `dark-mode`)
BANNED: Domain state or business logic in globals — globals hold tokens, strings, and routing only
BANNED: A single `Theme` mega-global — one file, one concern (see uiux/tokens.md)
